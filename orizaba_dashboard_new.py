...
def convert_parsed_data_to_dash_format(parsed_data):
    ...
    for sensor in parsed_data['sensors']:
        ...
        if sensor_name == 'IMU':
            dashboard_data['tilt_angle'] = sensor.get('tilt_angle', 0.0)
        ...
        elif sensor_name == "Rocket_State":
            # Convert state number to string
            dashboard_data['rocket_state'] = str(sensor.get('state', 0))
        ...
        while self.running:
            ...
            try:
                if ser.in_waiting:
                    ...
                    if parsed_result:
                            print("Successfully parsed packet data!")
...
class SensorDashboard(QMainWindow):
    ...
    def update_with_serial_data(self, data):
        ...
        # Update state display with rocket state names
        state_names = {
            "1": "IDLE",
            "2": "BOOST", 
            "3": "BURNOUT",
        ...
        state_name = state_names.get(str(state), "UNKNOWN")
        self.state.value_label.setText(state_name)
