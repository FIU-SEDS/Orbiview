...
def read_latest_data():
    ...
    try:
        ...
        # Map the new CSV columns to the expected format
        return (
            latest_row.get('linear_accel_x', 0),      # acceleration_x
            latest_row.get('linear_accel_y', 0),      # acceleration_y  
            latest_row.get('linear_accel_z', 0),      # acceleration_z
            latest_row.get('tilt_angle', 0),          # gyro_x (using tilt_angle)
            latest_row.get('heading', 0),             # gyro_y (using heading)
            latest_row.get('z_axis_g_force', 0),      # gyro_z (using g_force)
            latest_row.get('time_elapsed', 0),        # time_elapsed
            latest_row.get('rocket_state', 1),        # rocket_state
            -50,  # rssi (not available, use default)
            10    # signal_to_noise (not available, use default)
        )
    except Exception as e:
        print(f"Error reading CSV: {e}")
        print(f"CSV columns: {df.columns.tolist() if 'df' in locals() else 'Unknown'}")
        return None, None, None, None, None, None, None, None, None, None
...
app.layout = html.Div([
    ...
    html.Div([
        ...
        html.Div([
            ...
            'border': '2px solid white','margin-left': '20px', 'opacity': '0.25'
        })
...
def update_progress(n):
    ...
    # Convert state to int and map to original numbering (reversed)
    try:
        state_int = int(str(state).strip())
        # Map our states (1-8) to original progress states (7-1)
        progress_state = 8 - state_int + 1
    except (ValueError, TypeError):
        progress_state = 7  # Default
    
    # Map states to progress percentages (original mapping)
    stage_progress = {
        7: 100, 6: 84, 5: 70, 4: 56, 3: 42, 2: 28, 1: 17
    }
    
    progress_height = stage_progress.get(progress_state, 17)
...
@app.callback(
    ...
    dash.dependencies.Output('velocity(mph)', 'children'),
...
def update_data(n):
    ...
    # Get altitude from CSV - we need to modify read_latest_data to include altitude
    csv_file = find_latest_csv()
    altitude = 0
    velocity = 0
    ...
    if csv_file:
        try:
            ...
            if not df.empty:
                latest_row = df.iloc[-1]
                altitude = latest_row.get('altitude', 0)
                velocity = latest_row.get('linear_velocity_z', 0) * 2.237  # Convert ft/s to mph
        except:
            pass
    ...
    velocity_str = f"{velocity:.2f} MPH" if velocity is not None else "N/A"
    
    return altitude_str, velocity_str, elapsed_time
